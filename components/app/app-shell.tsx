    ...
    <div className="min-h-screen bg-canvas">
      <div className="flex min-h-screen">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={cn(
                    "flex items-center gap-3 rounded-md border border-transparent px-3 py-2 text-sm font-medium text-muted transition hover:bg-surface-muted hover:text-ink",
                    active && "border-line bg-surface-muted text-ink"
                  )}
                >
        ...
        <div className="flex min-h-screen flex-1 flex-col">
          <header className="border-b border-line bg-surface/95 px-4 py-3 sm:px-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="hidden h-9 items-center gap-2 rounded-md border border-line bg-surface px-2.5 text-sm text-muted transition hover:bg-surface-muted hover:text-ink md:inline-flex"
                >
                ...
                <button
                  ...
                  className="hidden h-9 items-center gap-2 rounded-md border border-line bg-surface px-2.5 text-sm text-muted transition hover:bg-surface-muted hover:text-ink sm:inline-flex"
                ...
                <button
                  type="button"
                  onClick={toggleTheme}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-line bg-surface text-muted transition hover:bg-surface-muted hover:text-ink"
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 rounded-md border border-line bg-surface px-2 py-1.5 transition hover:bg-surface-muted"
                  >
                    <div className="flex h-8 w-8 items-center justify-center rounded-full border border-line bg-surface-muted text-xs font-semibold text-ink">
                      {initials || "Q"}
                    </div>
                    ...
                    <MenuSquare className="h-3.5 w-3.5 text-muted" />
                  </button>

                  {menuOpen && (
                    <div className="absolute right-0 mt-2 w-48 rounded-lg border border-line bg-surface p-2 shadow-soft">
                      <Link
                        href="/settings"
                        className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-ink transition hover:bg-surface-muted"
            ...
            <div className="border-b border-line bg-surface px-4 py-2.5 sm:px-6">
              <div className="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-2 text-muted">
                  ...
                  <p>
                    Connect X to publish the same post everywhere with Voice DNA adaptation.
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <Link
                    href="/settings#connections"
                    className="inline-flex h-8 items-center justify-center rounded-md bg-brand px-3 text-sm font-medium text-white transition hover:bg-brand/90"
                  >
                    Connect X
                  </Link>
                  <button
                    ...
                    className="inline-flex h-8 items-center justify-center rounded-md px-2.5 text-sm font-medium text-muted transition hover:bg-surface-muted hover:text-ink"
                  >
                    Dismiss
                  </button>
                </div>
      ...
      {paletteOpen && (
        <div
          className="fixed inset-0 z-50 bg-slate-950/25 px-4 py-16 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"
          aria-label="Command palette"
        >
          <div className="mx-auto max-w-2xl overflow-hidden rounded-lg border border-line bg-surface shadow-soft">
            ...
            <div className="max-h-[420px] overflow-y-auto p-2">
              {filteredCommands.length === 0 ? (
                <div className="rounded-md px-4 py-6 text-sm text-muted">
                ...
                filteredCommands.map((command) => (
                  <button
                    ...
                    className="flex w-full items-start justify-between gap-3 rounded-md px-4 py-3 text-left transition hover:bg-surface-muted"
                  >
                    <div>
      ...
      {shortcutHelpOpen && (
        ...
        >
          <div className="mx-auto max-w-xl rounded-lg border border-line bg-surface p-6 shadow-soft">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-line text-muted transition hover:bg-surface-muted hover:text-ink"
            ...
            <div className="mt-6 space-y-3">
              {[
                ...
                <div
                  key={shortcut.label}
                  className="flex items-center justify-between rounded-md border border-line bg-surface-muted px-4 py-3"
                >
